...
(defun compute ()
  ...
  (let ((x (.x gl-global-invocation-id))
    ...
    (barrier)
    (dotimes (i 1)
      (let* ((y1 (3bgl-shaders::uint (mod y 8)))
             (y2 (3bgl-shaders::uint (floor (/ y 8))))
             (y8 (3bgl-shaders::uint (+ y2 (* 64 y1)))))
        ;; 512 pt
        ;; = 8pt fft
        ...
        (dotimes (i 8)
          (let* ((y512 (+ (* y 8) i))
                 (xy (ivec2 x y512))
                 (yy (3bgl-shaders::uint (+ (* 64 (mod y512 8))
                                            (floor (/ y512 8))))))
            (image-store out1 xy
                         (* 16 (vec4 (scratch lx yy 0 0)
                                     (scratch lx yy 0 0 t)
                                     0 0)))))
        ...
        ;; centered gaussian for initialization
        #++
        (dotimes (i 8)
          (let* ((xy (ivec2 x (+ i (* y 8))))
                 (c (vec2 256 256))
